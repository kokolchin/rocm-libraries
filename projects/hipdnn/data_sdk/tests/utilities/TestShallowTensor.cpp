...
TEST(TestShallowTensor, ConstructionAndShape)
{
    ...
    std::vector<int64_t> const dims = {1, 3, 2, 2}; // N C H W
    std::vector<int64_t> const strides = {12, 4, 2, 1}; // custom (not contiguous standard)
    ShallowTensor<float> const tensor(backing.data(), dims, strides);
...
TEST(TestShallowTensor, MemoryAccessHostOnly)
{
    ...
    std::vector<int64_t> const dims = {1, 1, 2, 3};
    std::vector<int64_t> const strides = {6, 6, 3, 1};
