...
TEST(TestKnob, CreateIntKnob)
{
    ...
    hipdnnBackendFlatbufferData_t const bufferData{buffer.data(), buffer.size()};
...
{
    KnobSetting const setting("test.knob", 42);
...
{
    KnobSetting const setting("test.knob", 3.14);
...
{
    KnobSetting const setting("test.knob", std::string("custom_value"));
...
{
    hipdnn_frontend::IntConstraint const constraint(0, 100, 1);
    std::string const str = constraint.toString();
...
{
    hipdnn_frontend::FloatConstraint const constraint(0.0, 1.0);
    std::string const str = constraint.toString();
...
{
    hipdnn_frontend::StringConstraint const constraint(100);
    std::string const str = constraint.toString();
...
TEST(TestKnob, ValidateIntKnobWithStep)
{
    ...
    // Invalid step value
    KnobSetting const invalidSetting(knob.knobId(), 15);
...
TEST(TestKnob, ValidateIntKnobWithValidValues)
{
    ...
    {
        KnobSetting const setting(knob.knobId(), validVal);
        EXPECT_EQ(knob.validate(setting).code, ErrorCode::OK);
    }

    // Invalid value
    KnobSetting const invalidSetting(knob.knobId(), 3);
    auto err = knob.validate(invalidSetting);
    EXPECT_EQ(err.code, ErrorCode::INVALID_VALUE);
...
TEST(TestKnob, ValidateFloatKnobOutOfRange)
{
    ...
    // Value below min
    KnobSetting const setting1(knob.knobId(), -0.1);
    auto err = knob.validate(setting1);
    EXPECT_EQ(err.code, ErrorCode::INVALID_VALUE);

    // Value above max
    KnobSetting const setting2(knob.knobId(), 1.1);
    err = knob.validate(setting2);
    EXPECT_EQ(err.code, ErrorCode::INVALID_VALUE);
}
...
TEST(TestKnob, ValidateStringKnobWithValidValues)
{
    ...
    {
        KnobSetting const setting(knob.knobId(), std::string(validVal));
        EXPECT_EQ(knob.validate(setting).code, ErrorCode::OK);
    }

    // Invalid value
    KnobSetting const invalidSetting(knob.knobId(), std::string("invalid"));
    auto err = knob.validate(invalidSetting);
    EXPECT_EQ(err.code, ErrorCode::INVALID_VALUE);
    EXPECT_NE(err.err_msg.find("not in the list of valid values"), std::string::npos);
}
...
TEST(TestKnob, ValidateStringKnobMaxLength)
{
    ...
    // Exceeds max length
    KnobSetting const invalidSetting(knob.knobId(), std::string("12345678901"));
    auto err = knob.validate(invalidSetting);
    EXPECT_EQ(err.code, ErrorCode::INVALID_VALUE);
...
TEST(TestKnob, ToStringIntKnob)
{
    ...
    std::string const str = knob.toString();
...
TEST(TestKnobSetting, ToStringIntSetting)
{
    ...
    std::string const str = setting.toString();
...
{
    KnobSetting const setting("test.knob", std::string("custom"));
...
{
    KnobSetting const setting("test.knob", std::string(""));
...
{
    KnobSetting const setting("test.knob", static_cast<int64_t>(-100));
...
{
    KnobSetting const intSetting("test.int", static_cast<int64_t>(0));
    KnobSetting const floatSetting("test.float", 0.0);
...
{
    int64_t const largeValue = 9223372036854775807LL; // INT64_MAX
    KnobSetting const setting("test.knob", largeValue);
...
{
    std::string const longString(1000, 'a');
    KnobSetting const setting("test.knob", longString);
...
{
    std::string const specialChars = "Test\nwith\ttabs\rand\"quotes\"";
    KnobSetting const setting("test.knob", specialChars);
...
{
    KnobSetting const setting1("knob_a", 1);
    KnobSetting const setting2("knob_b", 2);
...
{
    hipdnnBackendFlatbufferData_t const bufferData{nullptr, 0};
    auto [error, knob] = hipdnn_frontend::Knob::tryFromFlatbuffer(bufferData);
...
{
    uint8_t data = 0;
    hipdnnBackendFlatbufferData_t const bufferData{&data, 0};
    auto [error, knob] = hipdnn_frontend::Knob::tryFromFlatbuffer(bufferData);
...
TEST(TestKnob, InvalidFlatbufferDataReturnsError)
{
    ...
    hipdnnBackendFlatbufferData_t const bufferData{garbageData.data(), garbageData.size()};
    auto [error, knob] = hipdnn_frontend::Knob::tryFromFlatbuffer(bufferData);
...
{
    hipdnn_frontend::EmptyConstraint const constraint;
    std::string const str = constraint.toString();
...
TEST(TestEmptyConstraint, ValidatesAnyIntValue)
{
    ...
    KnobSetting const setting1("test", static_cast<int64_t>(0));
    EXPECT_EQ(constraint.validateKnobSetting(setting1).code, ErrorCode::OK);

    KnobSetting const setting2("test", static_cast<int64_t>(-1000000));
    EXPECT_EQ(constraint.validateKnobSetting(setting2).code, ErrorCode::OK);

    KnobSetting const setting3("test", static_cast<int64_t>(999999999));
...
TEST(TestEmptyConstraint, ValidatesAnyFloatValue)
{
    ...
    KnobSetting const setting1("test", 0.0);
    EXPECT_EQ(constraint.validateKnobSetting(setting1).code, ErrorCode::OK);

    KnobSetting const setting2("test", -1e10);
    EXPECT_EQ(constraint.validateKnobSetting(setting2).code, ErrorCode::OK);

    KnobSetting const setting3("test", 1e10);
    EXPECT_EQ(constraint.validateKnobSetting(setting3).code, ErrorCode::OK);
}
...
TEST(TestEmptyConstraint, ValidatesAnyStringValue)
{
    ...
    KnobSetting const setting1("test", std::string(""));
    EXPECT_EQ(constraint.validateKnobSetting(setting1).code, ErrorCode::OK);

    KnobSetting const setting2("test", std::string("any string value"));
    EXPECT_EQ(constraint.validateKnobSetting(setting2).code, ErrorCode::OK);

    std::string const longString(10000, 'x');
    KnobSetting const setting3("test", longString);
    EXPECT_EQ(constraint.validateKnobSetting(setting3).code, ErrorCode::OK);
}
