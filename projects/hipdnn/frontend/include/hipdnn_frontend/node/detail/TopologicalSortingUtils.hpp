...
{
    size_t const nodeCount = structure.adjacencyList.size();
...
inline std::vector<size_t> performTopologicalSort(const GraphStructure& structure)
{
    ...
    {
        size_t const current = zeroInDegree.front();
        zeroInDegree.pop();
...
inline int countConnectedComponents(const GraphStructure& structure)
{
    ...
    for(size_t start = 0; start < nodeCount; ++start)
    {
        ...
        while(!stack.empty())
        {
            size_t const current = stack.top();
            stack.pop();
    ...
    performTopologicalSortWithComponentDetection(const GraphStructure& structure)
{
    ...
    int const componentCount = countConnectedComponents(structure);
    bool const hasCycle = detectCycle(topologicalOrder, structure);
