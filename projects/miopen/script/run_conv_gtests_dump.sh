...
#   ./script/run_conv_gtests_dump.sh [build_dir] [jobs]
...
cmake -S "${SRC_DIR}" -B "${BUILD_DIR}" -DCMAKE_BUILD_TYPE=Release -DMIOPEN_BACKEND=HIP -DCMAKE_C_COMPILER="${CC}" -DCMAKE_CXX_COMPILER="${CXX}"
...
echo "[INFO] Building ${#GTEST_TARGETS[@]} conv gtest targets (jobs=${JOBS})..."

cmake --build "${BUILD_DIR}" --parallel "${JOBS}" --target "${GTEST_TARGETS[@]}"
