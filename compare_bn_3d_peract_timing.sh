...
#      cmake .. && cmake --build . --target test_bn_3d_peract_test
...
#
# Examples:
#   # Use default paths (searches in BUILD_DIR, which defaults to "build")
#   BUILD_DIR=build ./compare_bn_3d_peract_timing.sh 5
#
...
#   ./compare_bn_3d_peract_timing.sh 5 "" /path/to/new/binary
...
if [ ! -d "$BUILD_DIR" ]; then
    ...
    echo "Please set BUILD_DIR environment variable or build the project first"
    exit 1
fi
...
if [ -n "$OLD_CTEST_BINARY_ARG" ]; then
    ...
    cd "$BUILD_DIR"
    # Check if old ctest binary exists (if we can restore it)
    if [ -f "projects/miopen/test/test_bn_3d_peract_test" ]; then
        OLD_CTEST_BINARY="projects/miopen/test/test_bn_3d_peract_test"
        echo "Found old ctest binary: $OLD_CTEST_BINARY"
    elif [ -f "test_bn_3d_peract_test" ]; then
        OLD_CTEST_BINARY="test_bn_3d_peract_test"
        echo "Found old ctest binary: $OLD_CTEST_BINARY"
    else
        echo "Warning: Old ctest binary not found. Skipping old ctest timing."
        echo "To test old ctest, you can:"
        echo "  1. Build it in a separate build directory (e.g., build_old)"
...
if [ -n "$NEW_GTEST_BINARY_ARG" ]; then
    ...
    cd "$BUILD_DIR"
    # Check if new gtest binary exists
    if [ -f "projects/miopen/test/gtest/test_bn_3d_peract_test" ]; then
        NEW_GTEST_BINARY="projects/miopen/test/gtest/test_bn_3d_peract_test"
    elif [ -f "test_bn_3d_peract_test" ]; then
        NEW_GTEST_BINARY="test_bn_3d_peract_test"
    fi
    cd ..
fi
...
if [ -z "$NEW_GTEST_BINARY" ] || [ ! -f "$NEW_GTEST_BINARY" ]; then
    ...
    echo "  cd $BUILD_DIR && cmake .. && cmake --build . --target test_bn_3d_peract_test"
...
if [ -n "$OLD_CTEST_BINARY" ] && [ -f "$OLD_CTEST_BINARY" ]; then
    ...
    if [[ "$OLD_CTEST_BINARY" != /* ]]; then
        if [ -d "$BUILD_DIR" ]; then
            OLD_CTEST_BINARY="$BUILD_DIR/$OLD_CTEST_BINARY"
        fi
    fi
...
if [[ "$NEW_GTEST_BINARY" != /* ]]; then
    if [ -d "$BUILD_DIR" ]; then
        NEW_GTEST_BINARY="$BUILD_DIR/$NEW_GTEST_BINARY"
    fi
fi
# Get directory of binary for running
